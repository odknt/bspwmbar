...
#ifndef BSPWMBAR_UTIL_H
#define BSPWMBAR_UTIL_H
...
#define list_for_each_reverse(head, pos) for (pos = (head)->prev; pos != (head); pos = pos->prev)
#define list_entry(ptr, type, member)    ((type *)((char *)(ptr)-(char *)(&((type *)0)->member)))
#define list_empty(head)                 ((head)->next == (head))
#define list_count(head, num)            for (list_head *pos = (head)->next; pos != (head); pos = pos->next) num++
#define list_for_each_safe(head, pos, n) for (pos = (head)->next, n = pos->next; pos != (head); pos = n, n = pos->next)
